...
wxMemoryInputStream::wxMemoryInputStream(const void *data, size_t len)
{
    ...
    m_i_streambuf->SetIntPosition(0);     m_i_streambuf->Fixed(true);
...
wxMemoryInputStream::wxMemoryInputStream(const wxMemoryOutputStream& stream)
{
    ...
    m_i_streambuf = new wxStreamBuffer(wxStreamBuffer::read);
    m_i_streambuf->SetBufferIO(len);     stream.CopyTo(m_i_streambuf->GetBufferStart(), len);
    m_i_streambuf->SetIntPosition(0);     m_i_streambuf->Fixed(true);
    m_length = len;
}
...
wxMemoryInputStream::InitFromStream(wxInputStream& stream, wxFileOffset lenFile)
{
    ...
    m_i_streambuf = new wxStreamBuffer(wxStreamBuffer::read);
    m_i_streambuf->SetBufferIO(len);     stream.Read(m_i_streambuf->GetBufferStart(), len);
    m_i_streambuf->SetIntPosition(0);     m_i_streambuf->Fixed(true);
