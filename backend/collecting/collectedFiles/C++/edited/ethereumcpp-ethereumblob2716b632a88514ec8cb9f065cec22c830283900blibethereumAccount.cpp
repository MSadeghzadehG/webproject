...
namespace
{
	...
	string const c_codeFromFile = "codeFromFile";  	string const c_storage = "storage";
    ...
    AccountMaskMap* o_mask, PrecompiledContractMap* o_precompiled, const fs::path& _configPath)
{
	...
	for (auto const& account: o)
	{
		...
		if (haveStorage || haveCode || haveNonce || haveBalance)
		{
            ...
            if (codePathIt != o.end())
            {
                ...
                if (codePathObj.type() == json_spirit::str_type)
                {
                    ...
                    if (codePath.is_relative())                          codePath = _configPath.parent_path() / codePath;
		...
		if (o_mask)
		{
			...
			if (!haveStorage && !haveCode && !haveNonce && !haveBalance && shouldNotExists) 				ret[a] = Account(0, 0);
		}
