...
bool wxBMPHandler::SaveFile(wxImage *image,
                            ...
                            bool verbose)
{
    return SaveDib(image, stream, verbose, true, false);
}
...

{
    ...
    unsigned row_padding = (4 - ((width * bpp + 7) / 8) % 4) % 4;     unsigned row_width = (width * bpp + 7) / 8 + row_padding; 
    struct
    {
                wxUint16  magic;                  wxUint32  filesize;               wxUint32  reserved;               wxUint32  data_offset;    
                wxUint32  bih_size;               wxUint32  width, height;          wxUint16  planes;                 wxUint16  bpp;                    wxUint32  compression;            wxUint32  size_of_bmp;            wxUint32  h_res, v_res;           wxUint32  num_clrs;               wxUint32  num_signif_clrs;    } hdr;

    wxUint32 hdr_size = 14 + 40;

    hdr.magic = wxUINT16_SWAP_ON_BE(0x4D42);
    ...
    hdr.planes = wxUINT16_SWAP_ON_BE(1);     hdr.bpp = wxUINT16_SWAP_ON_BE(bpp);
    hdr.compression = 0;     hdr.size_of_bmp = wxUINT32_SWAP_ON_BE(row_width * image->GetHeight());
    ...
    hdr.num_clrs = wxUINT32_SWAP_ON_BE(palette_size);     hdr.num_signif_clrs = 0;     
    if ( IsBmp )
    {
        if (                                    !stream.WriteAll(&hdr.magic, 2) ||
...
#if wxUSE_PALETTE
    wxScopedPtr<wxPalette> palette; #endif     wxScopedArray<wxUint8> rgbquad;     wxScopedPtr<wxImage> q_image;   
...
#else             wxQuantize::Quantize( *image, *q_image, NULL, quantize, 0,
                                  wxQUANTIZE_FILL_DESTINATION_IMAGE );
#endif         }
        else
        {
    ...
    for (y = image->GetHeight() -1; y >= 0; y--)
    {
        ...
        else if ((format == wxBMP_8BPP) ||                        (format == wxBMP_8BPP_PALETTE))
        {
            for (x = 0; x < width; x++)
            {
                pixel = 3*(y*width + x);
        ...
        else if ( format == wxBMP_8BPP_GREY )         {
            for (x = 0; x < width; x++)
            {
                pixel = 3*(y*width + x);
                           ...
                           bool verbose, bool IsBmp)
{
    ...
    if ( IsBmp )
    {
        ...
        #if 0             wxInt32 size = wxINT32_SWAP_ON_BE(dbuf[0]);
        #endif
    ...
    else
    {
        ...
        offset = wxInvalidOffset;         hdrSize = wxINT32_SWAP_ON_BE(dbuf[0]);
    }
...
{
        return LoadDib(image, stream, verbose, true);
}
...
bool wxBMPHandler::DoCanRead(wxInputStream& stream)
{
    ...
    if ( !stream.ReadAll(hdr, WXSIZEOF(hdr)) )             return false;
...
struct ICONDIRENTRY
{
    wxUint8         bWidth;                   wxUint8         bHeight;                  wxUint8         bColorCount;              wxUint8         bReserved;            
                                                  wxUint16        wPlanes;                  wxUint16        wBitCount;            
    wxUint32        dwBytesInRes;             wxUint32        dwImageOffset;        };

struct ICONDIR
{
    wxUint16     idReserved;       wxUint16     idType;           wxUint16     idCount;      };
...
#endif         {
            bResult = LoadDib(image, stream, verbose, false );
        }
...
{
    return CanReadICOOrCUR(&stream, 1 );
...
{
    return CanReadICOOrCUR(&stream, 2 );
}
...
{
    wxANIDecoder decoder;
    if (!decoder.Load(stream))          return wxNOT_FOUND;
...
static bool CanReadICOOrCUR(wxInputStream *stream, wxUint16 resourceType)
{
    ...
    return !iconDir.idReserved         && wxUINT16_SWAP_ON_BE(iconDir.idType) == resourceType         && iconDir.idCount; }
