...
bool wxGtkDataViewModelNotifier::Cleared()
{
    ...
    int count = m_internal->iter_n_children( NULL ); 
...
{
    gtk_tree_view_column_pack_end( column, m_renderer, TRUE );
}
...
{
        gtk_tree_view_column_pack_start(column, m_rendererIcon, FALSE );
                             ...
                             wxDataViewColumn *& column) const
{
    ...
    gint cell_y = 0;
        ...
        wxDataViewCtrl* self = const_cast<wxDataViewCtrl *>(this);     GtkTreeView* treeView = GTK_TREE_VIEW(self->GtkGetTreeView());
    ...
    {                                             
        ...
        if (path)
            item = wxDataViewItem(GTKPathToItem(path));
                for ( unsigned int i=0, cols=GetColumnCount(); i<cols; ++i )          {
            wxDataViewColumn* col = GetColumn(i);
