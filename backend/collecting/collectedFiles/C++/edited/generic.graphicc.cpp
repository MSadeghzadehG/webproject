...
#endif     wxCairoBitmapData( wxGraphicsRenderer* renderer, cairo_surface_t* bitmap );
    ~wxCairoBitmapData();
...
wxCairoBrushData::wxCairoBrushData( wxGraphicsRenderer* renderer )
    : wxCairoPenBrushBaseData(renderer, wxColour(), true )
{
    Init();
}
    ...
    : wxGraphicsBitmapData(renderer)
{
    ...
    wxBitmap bmpSource = bmp;      wxUint32* data = (wxUint32*)m_buffer;
...
{
    wxImage image(m_width, m_height, false );
