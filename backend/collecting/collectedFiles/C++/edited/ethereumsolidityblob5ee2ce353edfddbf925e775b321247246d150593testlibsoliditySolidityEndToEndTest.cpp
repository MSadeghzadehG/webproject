...
BOOST_AUTO_TEST_CASE(generic_delegatecall)
{
	...
	compileAndRun(sourceCode, 50, "Sender");
	u160 const c_senderAddress = m_contractAddress;
	BOOST_CHECK(m_sender != c_senderAddress); 	ABI_CHECK(callContractFunctionWithValue("doSend(address)", 11, c_receiverAddress), encodeArgs());
...
BOOST_AUTO_TEST_CASE(array_copy_different_packing)
{
	char const* sourceCode = R"(
		contract c {
			bytes8[] data1; 			bytes10[] data2; 			function test() returns (bytes10 a, bytes10 b, bytes10 c, bytes10 d, bytes10 e) {
				data1.length = 9;
...
BOOST_AUTO_TEST_CASE(array_copy_target_simple)
{
	char const* sourceCode = R"(
		contract c {
			bytes8[9] data1; 			bytes17[10] data2; 			function test() returns (bytes17 a, bytes17 b, bytes17 c, bytes17 d, bytes17 e) {
				for (uint i = 0; i < data1.length; ++i)
					data1[i] = bytes8(uint64(i));
				data2[8] = data2[9] = 2;
				data2 = data1;
...
BOOST_AUTO_TEST_CASE(array_copy_target_leftover2)
{
			char const* sourceCode = R"(
		contract c {
			bytes8[4] data1; 			bytes10[6] data2; 			function test() returns (bytes10 r1, bytes10 r2, bytes10 r3) {
				data1[0] = 1;
				data1[1] = 2;
				data1[2] = 3;
				data1[3] = 4;
...
BOOST_AUTO_TEST_CASE(external_array_args)
{
	...
	bytes params = encodeArgs(
		1, 2, 3, 4, 5, 6, 7, 8, 		32 * (8 + 1 + 5 + 1 + 1 + 1), 		21, 22, 23, 24, 25, 		0, 1, 2, 		3, 		11, 12, 13 		);
...
BOOST_AUTO_TEST_CASE(array_copy_calldata_storage)
{
	...
	ABI_CHECK(callContractFunction("store(uint256[9],uint8[3][])", encodeArgs(
		21, 22, 23, 24, 25, 26, 27, 28, 29, 		u256(32 * (9 + 1)),
		4, 		1, 2, 3, 		11, 12, 13, 		21, 22, 23, 		31, 32, 33 	)), encodeArgs(32));
...
BOOST_AUTO_TEST_CASE(struct_assign_reference_to_struct)
{
	char const* sourceCode = R"(
		contract test {
			...
			{
				testStruct x = data1; 				data2 = data1; 
				ret_local = x.m_value; 				ret_global = data2.m_value; 
				x.m_value = 3;
				data3 = x; 				ret_global3 = data3.m_value; 				ret_global1 = data1.m_value; 			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(struct_delete_member)
{
	char const* sourceCode = R"(
		contract test {
			...
			{
				testStruct x = data1; 				x.m_value = 4;
				delete x.m_value;
		...
		contract BinarySearch {
		  ...
		  function find(uint[] storage _data, uint _begin, uint _len, uint _value) private returns (uint o_position) {
			if (_len == 0 || (_len == 1 && _data[_begin] != _value))
			  return uint(-1); 			uint halfLen = _len / 2;
...
BOOST_AUTO_TEST_CASE(fixed_arrays_as_return_type)
{
	...
	compileAndRun(sourceCode, 0, "B");
	ABI_CHECK(callContractFunction("f()"), encodeArgs(
		u256(2), u256(3), u256(4), u256(5), u256(6), 		u256(1000), u256(1001), u256(1002), u256(1003), u256(1004)) 	);
}
		...
		contract C {
			function f() returns (address) {
				return recover(
					...
					0, 					0x6944c77849b18048f6abe0db8084b0d0d0689cdddb53d2671c36967b58691ad4,
