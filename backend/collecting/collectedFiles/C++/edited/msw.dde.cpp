...
wxDDEServer::~wxDDEServer()
{
    ...
    while (node)
    {
        ...
        connection->OnDisconnect();         node = next;
    }
...
wxConnectionBase *wxDDEServer::OnAcceptConnection(const wxString& )
{
...
wxDDEClient::~wxDDEClient()
{
    ...
    {
        wxDDEConnection *connection = node->GetData();
        delete connection;          node = m_connections.GetFirst();
    }
}

bool wxDDEClient::ValidHost(const wxString& )
{
    return true;
}
                               ...
                               wxIPCFormat format)
{
    ...
    m_sendingData = data;      m_dataSize = size;
        ...
        case XTYP_DISCONNECT:
            {
                ...
                if (connection)
                {
                    ...
                    {
                        DDEDeleteConnection(hConv);                          return (DDERETURN)(DWORD)true;
                    }
                }
                break;
            }
                    ...
                    if (data)
                    {
                      if (user_size == wxNO_LEN)
                        switch (wFmt)
                        {
                          case wxIPC_TEXT:
                          case wxIPC_UTF8TEXT:
                            user_size = strlen((const char*)data) + 1;                              break;
                          case wxIPC_UNICODETEXT:
                            user_size = (wcslen((const wchar_t*)data) + 1) * sizeof(wchar_t);                              break;
                          default:
                            user_size = 0;
                        }
