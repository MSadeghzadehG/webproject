...
wxImage wxGDIPlusBitmapData::ConvertToImage() const
{
    ...
    unsigned char *imgRGB = img.GetData();        unsigned char *imgAlpha = img.GetAlpha();     const BYTE* pixels = static_cast<const BYTE*>(bitmapData.Scan0);
    for( UINT y = 0; y < h; y++ )
    {
        for( UINT x = 0; x < w; x++ )
        {
            ARGB c = reinterpret_cast<const ARGB*>(pixels)[x];
            *imgRGB++ = (c >> 16) & 0xFF;              *imgRGB++ = (c >> 8) & 0xFF;               *imgRGB++ = (c >> 0) & 0xFF;               if ( imgAlpha )
                *imgAlpha++ = (c >> 24) & 0xFF;
        }
...
void wxGDIPlusContext::BeginLayer(wxDouble )
{
    }
                                        ...
                                        wxDouble x, wxDouble y )
{
   if (m_composition == wxCOMPOSITION_DEST)
    ...
    m_context->DrawString
               (
                    str.wc_str(*wxConvUI),                      -1,                                         fontData->GetGDIPlusFont(),
                    PointF(x, y),
