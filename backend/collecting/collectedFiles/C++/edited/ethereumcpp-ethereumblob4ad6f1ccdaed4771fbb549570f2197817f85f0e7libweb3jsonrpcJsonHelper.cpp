...
TransactionSkeleton toTransactionSkeleton(Json::Value const& _json)
{
    ...
    if (!_json["data"].empty())							        ret.data = jsToBytes(_json["data"].asString(), OnFailed::Throw);
...
dev::eth::LogFilter toLogFilter(Json::Value const& _json)
{
    ...
    if (!_json["topics"].empty())
        for (unsigned i = 0; i < _json["topics"].size(); i++)
        {
            ...
            else if (!_json["topics"][i].isNull())                 filter.topic(i, jsToFixed<32>(_json["topics"][i].asString()));
        }
    return filter;
}
