...
void wxGenericPrintSetupDialog::Init(wxPrintData* data)
{
    ...
    if (res >= 0 && errors.GetCount() == 0)
    {
        ...
        for (i = 0; i < output.GetCount(); i++)
        {
            ...
            wxString tmp = tok.GetNextToken();             if (tmp != wxT("device"))
                break;              tmp = tok.GetNextToken();                      if (tmp != wxT("for"))
                break;              tmp = tok.GetNextToken();                      if (tmp[tmp.length()-1] == wxT(':'))
                tmp.Remove(tmp.length()-1,1);
            wxString name = tmp;
            item.SetText( name );
            item.SetId( m_printerListCtrl->InsertItem( item ) );
            tmp = tok.GetNextToken();                      item.SetColumn( 2 );
            item.SetText( tmp );
            ...
            {
                tmp = output2[0];                 int pos = tmp.Find( wxT('.') );
                if (pos != wxNOT_FOUND)
                    tmp.Remove( (size_t)pos, tmp.length()-(size_t)pos );
                wxStringTokenizer tok2( tmp, wxT(" ") );
                tmp = tok2.GetNextToken();                  tmp = tok2.GetNextToken();                  tmp = wxEmptyString;
...
void wxGenericPageSetupDialog::OnPrinter(wxCommandEvent& WXUNUSED(event))
{
    ...
    GetPageSetupData().CalculatePaperSizeFromId(); 
