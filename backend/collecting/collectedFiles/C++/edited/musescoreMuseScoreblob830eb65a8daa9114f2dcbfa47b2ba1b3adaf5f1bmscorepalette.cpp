         ...
         && (ev->pos() - dragStartPosition).manhattanLength() > QApplication::startDragDistance())
            ...
            if (cell && cell->element) {
                  ...
                  if (!(_readOnly || filterActive) && (ev->modifiers() & Qt::ShiftModifier)) {
                        dragCells = cells;                              da = Qt::MoveAction;
                        }
                  else
                        da = Qt::CopyAction;
                  Qt::DropAction a = drag->exec(da);
                  if (da == Qt::MoveAction && a != da)
                        cells = dragCells;                        update();
                  }
            }
      else {
...
void Palette::applyPaletteElement(PaletteCell* cell)
      ...
      if (score == 0)
            return;
      Selection sel = score->selection();             if (sel.isNone())
            return;
      ...
      if (viewer->mscoreState() != STATE_EDIT
         ...
         && viewer->mscoreState() != STATE_TEXT_EDIT) {             score->startCmd();
            }
      if (sel.isList()) {
            ChordRest* cr1 = sel.firstChordRest();
            ChordRest* cr2 = sel.lastChordRest();
            bool addSingle = false;                   if (cr1 && cr2 == cr1) {
                                    addSingle = true;
                  }
            ...
            if (element->type() == ElementType::BAR_LINE
                ...
                || (element->type() == ElementType::ICON
                  ...
                  Segment* startSegment = sel.startSegment();
                  Segment* endSegment = sel.endSegment();                   for (Segment* s = startSegment; s && s != endSegment; s = s->next1()) {
      ...
      if (viewer->mscoreState() != STATE_EDIT
         && viewer->mscoreState() != STATE_LYRICS_EDIT
         && viewer->mscoreState() != STATE_HARMONY_FIGBASS_EDIT
         && viewer->mscoreState() != STATE_TEXT_EDIT) {             score->endCmd();
...
static void writeFailed(const QString& path)
      {
      QString s = qApp->translate("Palette", "Writing Palette File\n%1\nfailed: ").arg(path);       QMessageBox::critical(mscore, qApp->translate("Palette", "Writing Palette File"), s);
      }
...
void Palette::read(XmlReader& e)
      ...
      while (e.readNextStartElement()) {
            ...
            else if (t == "drumPalette")                        e.skipCurrentElement();
            else if (t == "Cell") {
                  PaletteCell* cell = new PaletteCell;
...
void Palette::actionToggled(bool )
      {
      selectedIdx = -1;
      int nn = ccp()->size();
      for (int n = 0; n < nn; ++n) {
