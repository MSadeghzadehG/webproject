...
void CodeFragment::constructOperation(sp::utree const& _t, CompilerState& _s)
{
	...
	else
	{
		...
		else if (us == "ALLOC")
		{
			...
			m_asm.append(Instruction::MSIZE); 			m_asm.append(code[0].m_asm, 1);	  			m_asm.append(Instruction::DUP1);
			m_asm.append(Instruction::ISZERO);			m_asm.appendJumpI(end);
			m_asm.append(u256(1));
			m_asm.append(Instruction::DUP2);  			m_asm.append(Instruction::SUB);	  			m_asm.append(u256(0x1f));         			m_asm.append(Instruction::NOT);	  			m_asm.append(Instruction::AND);	  			m_asm.append(Instruction::MSIZE); 			m_asm.append(Instruction::ADD);
			m_asm.append(Instruction::MLOAD); 			m_asm.append(Instruction::POP);	  			m_asm.append(end);
