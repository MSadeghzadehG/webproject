...
string OptionContainer::format_option_help(unsigned int indent ) const {
  stringstream ss;
...
string OptionParser::format_help() const {
  ...
  for (list<OptionGroup const*>::const_iterator it = _groups.begin(); it != _groups.end(); ++it) {
    ...
    if (group.description() != "") {
      unsigned int malus = 4;       ss << str_format(group.description(), 4, cols() - malus) << endl;
    }
...
string Option::format_option_help(unsigned int indent ) const {
...
string Option::format_help(unsigned int indent ) const {
  stringstream ss;
