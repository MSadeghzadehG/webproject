...
void Segment::read(XmlReader& e)
      ...
      while (e.readNextStartElement()) {
            ...
            else if (tag == "trailingSpace")                            e.readDouble();
            else
                  e.unknown();
            }
      }
...
Element* Segment::firstInNextSegments(int activeStaff)
      ...
      if (!seg) {             seg = score()->firstSegment(SegmentType::All);
...
Spanner* Segment::firstSpanner(int activeStaff)
      ...
      if (range.first != range.second){             for (auto i = range.first; i != range.second; ++i) {
                  Spanner* s = i->second;
...
Spanner* Segment::lastSpanner(int activeStaff)
      ...
      std::multimap<int, Spanner*> mmap = score()->spanner();
      auto range = mmap.equal_range(tick());
      if (range.first != range.second){             for (auto i = --range.second; i != range.first; --i) {
                  Spanner* s = i->second;
...
Element* Segment::lastInPrevSegments(int activeStaff)
      ...
      if (!seg) {             if (activeStaff -1 < 0)                   return 0;

            re = 0;
            seg = score()->lastSegment();
            while (true) {
                  if (seg->segmentType() == SegmentType::EndBarLine)
                        score()->inputState().setTrack( (activeStaff -1) * VOICES ); 
