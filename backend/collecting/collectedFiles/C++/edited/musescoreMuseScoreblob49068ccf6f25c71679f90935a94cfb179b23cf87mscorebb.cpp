...
bool BBFile::read(const QString& name)
      ...
      if (a[idx] == 1) {                        qDebug("Skip 0x%02x at 0x%04x", a[idx], idx);
            ++idx;
            }
...
void BBFile::convertTrack(Score* score, BBTrack* track, int staffIdx)
      ...
      for (int voice = 0; voice < voices; ++voice) {
            ...
            for (; i != el.end();) {
                  ...
                  while (!notes.isEmpty()) {
                        ...
                        while (restLen > 0) {
                              ...
                              if (measure == 0 || (ctick >= (measure->tick() + measure->ticks()))) {                                           ctick += len;
                                    restLen -= len;
                                    break;
                                    }
            ...
            if (voice == 0) {
                  Measure* measure = score->tick2measure(ctick);
                  if (measure && (ctick < (measure->tick() + measure->ticks()))) {                               TDuration d;
...
void BBTrack::quantize(int startTick, int endTick, EventList* dst)
      ...
      if (mintick <= MScore::division / 16)                    mintick = MScore::division / 16;
...
int BBTrack::separateVoices(int )
      {
      return 1;
      }
}
