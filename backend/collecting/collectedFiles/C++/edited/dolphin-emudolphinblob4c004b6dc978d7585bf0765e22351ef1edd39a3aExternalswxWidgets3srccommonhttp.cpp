...
wxString wxHTTP::GenerateAuthString(const wxString& user, const wxString& pass) const
{
    ...
    while (len >= 3) {         buf << wxString::Format(wxT("%c%c"), base64[(from[0] >> 2) & 0x3f], base64[((from[0] << 4) & 0x30) | ((from[1] >> 4) & 0xf)]);
        buf << wxString::Format(wxT("%c%c"), base64[((from[1] << 2) & 0x3c) | ((from[2] >> 6) & 0x3)], base64[from[2] & 0x3f]);
        from += 3;
        len -= 3;
    }
    if (len > 0) {         buf << wxString::Format(wxT("%c"), base64[(from[0] >> 2) & 0x3f]);
        if (len == 1) {
...
#else     const size_t len = data.length();
...
wxInputStream *wxHTTP::GetInputStream(const wxString& path)
{
    ...
    m_lastError = wxPROTO_CONNERR;      if (!m_addr)
        return NULL;
