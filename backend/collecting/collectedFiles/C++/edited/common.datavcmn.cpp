...
#endif     else if (value1.GetType() == wxT("bool"))
    {
...
{
    unsigned int pos1 = wxPtrToUInt(item1.GetID());      unsigned int pos2 = wxPtrToUInt(item2.GetID());  
    if (ascending)
       return pos1 - pos2;
    else
       return pos2 - pos1;
...
wxDataViewCustomRendererBase::WXCallRender(wxRect rectCell, wxDC *dc, int state)
{
    ...
    else         col = GetOwner()->GetOwner()->GetForegroundColour();
