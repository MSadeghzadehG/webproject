...
struct ImportRequirements
{
	...
	enum
	{
		ValidSeal = 1, 		UncleBasic = 4, 		TransactionBasic = 8, 		UncleSeals = 16, 		TransactionSignatures = 32, 		Parent = 64, 		UncleParent = 128, 		PostGenesis = 256, 		CheckUncles = UncleBasic | UncleSeals, 		CheckTransactions = TransactionBasic | TransactionSignatures, 		OutOfOrderChecks = ValidSeal | CheckUncles | CheckTransactions, 		InOrderChecks = Parent | UncleParent, 		Everything = ValidSeal | CheckUncles | CheckTransactions | Parent | UncleParent,
		None = 0
	};
};
...
{
	uint64_t hashes = 0;			uint64_t ms = 0;				u256 rate() const { return ms == 0 ? 0 : hashes * 1000 / ms; }
};
