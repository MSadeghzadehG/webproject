...
void JackAudio::timebase(jack_transport_state_t state, jack_nframes_t , jack_position_t *pos, int , void *arg)
      {
      JackAudio* audio = (JackAudio*)arg;
      if (!audio->seq->score()) {
...
Transport JackAudio::getState()
      ...
      switch (transportState) {
            ...
            case JackTransportStarting: return seq->isPlaying()?Transport::PLAY:Transport::STOP;            default:
...
void JackAudio::setTimebaseCallback()
      {
      int errCode = jack_set_timebase_callback(client, 0, timebase, this);       if (errCode == 0) {
            if (MScore::debugMode)
