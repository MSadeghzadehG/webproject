...
wxRendererMac::GetSplitterParams(const wxWindow *win)
{
    ...
    if ( win->HasFlag(wxSP_3DSASH) )
        GetThemeMetric( kThemeMetricPaneSplitterHeight, &sashWidth );     else if ( win->HasFlag(wxSP_NOSASH) )         sashWidth = 0;
    else         sashWidth = 1;
#else     if ( win->HasFlag(wxSP_3DSASH) )
        GetThemeMetric( kThemeMetricPaneSplitterHeight, &sashWidth );
    else if ( win->HasFlag(wxSP_NOSASH) )
        sashWidth = 0;
    else         GetThemeMetric( kThemeMetricSmallPaneSplitterHeight, &sashWidth );
#endif 
