...
class ComparableTimSort {
	...
	private static int countRunAndMakeAscending (Object[] a, int lo, int hi) {
		...
		if (runHi == hi) return 1;
				...
				if (((Comparable)a[runHi++]).compareTo(a[lo]) < 0) { 			while (runHi < hi && ((Comparable)a[runHi]).compareTo(a[runHi - 1]) < 0)
				runHi++;
			reverseRange(a, lo, runHi);
		} else { 			while (runHi < hi && ((Comparable)a[runHi]).compareTo(a[runHi - 1]) >= 0)
				runHi++;
		}
	...
	private void mergeLo (int base1, int len1, int base2, int len2) {
		if (DEBUG) assert len1 > 0 && len2 > 0 && base1 + len1 == base2;
				...
				Object[] a = this.a; 		Object[] tmp = ensureCapacity(len1);
		...
		int minGallop = this.minGallop; 		outer:
		while (true) {
	...
	private void mergeHi (int base1, int len1, int base2, int len2) {
		if (DEBUG) assert len1 > 0 && len2 > 0 && base1 + len1 == base2;
				...
				Object[] a = this.a; 		Object[] tmp = ensureCapacity(len2);
