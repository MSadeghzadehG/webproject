	...
	private static InsnNode convertInvoke(MethodNode mth, InsnNode insn) {
		MethodInfo callMth = ((InvokeNode) insn).getCallMth();
			...
			try {
				...
				int constrIndex = -1;  												if (chain.size() > 1 && chain.get(0).getType() == InsnType.CONSTRUCTOR) {
					constrIndex = 0;
				...
				if (constrIndex != -1) {  					ConstructorInsn constr = (ConstructorInsn) chain.get(constrIndex);
					if (constr.getClassType().getFullName().equals(Consts.CLASS_STRING_BUILDER)) {
						...
						InsnNode argInsn;
						if (constrIndex > 0) {  							InsnWrapArg iwa;
							if (constrIndex == 2
						...
						for (; argInd < len; argInd++) {  							concatInsn.addArg(chain.get(argInd).getArg(1));
						}
