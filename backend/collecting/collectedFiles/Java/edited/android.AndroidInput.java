...
public class AndroidInput implements Input, OnKeyListener, OnTouchListener {
	...
	public void getRotationMatrix (float[] matrix) {
		...
		else 			SensorManager.getRotationMatrix(matrix, null, accelerometerValues, magneticFieldValues);
	}
	...
	void registerSensorListeners () {
		...
		if (config.useRotationVectorSensor){
			...
			if (rotationVectorSensors.size() > 0){
				...
				for (Sensor sensor : rotationVectorSensors){ 					if (sensor.getVendor().equals("Google Inc.") && sensor.getVersion() == 3){
