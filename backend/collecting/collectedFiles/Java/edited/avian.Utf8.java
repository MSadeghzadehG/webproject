...
public class Utf8 {
	...
	public static byte[] encode (char[] s16, int offset, int length) {
		...
		for (int i = offset; i < offset + length; ++i) {
			...
			if (c == '\u0000') { 				buf.write(0);
				buf.write(0);
			} else if (c < 0x080) { 				buf.write(c);
			} else if (c < 0x0800) { 				buf.write(0x0c0 | (c >>> 6));
				buf.write(0x080 | (c & 0x03f));
			} else { 				buf.write(0x0e0 | ((c >>> 12) & 0x0f));
	...
	public static Object decode (byte[] s8, int offset, int length) {
		...
		while (i < offset + length) {
			int x = s8[i++];
			if ((x & 0x080) == 0x0) { 				if (x == 0) ++i; 				cram(buf, j++, x);
			} else if ((x & 0x0e0) == 0x0c0) { 				if (!isMultiByte) {
					buf = widen(buf, j, length - 1);
					isMultiByte = true;
				}
				int y = s8[i++];
				cram(buf, j++, ((x & 0x1f) << 6) | (y & 0x3f));
			} else if ((x & 0x0f0) == 0x0e0) { 				if (!isMultiByte) {
