...
public class Quaternion implements Serializable {
	...
	public Quaternion setEulerAnglesRad (float yaw, float pitch, float roll) {
		...
		x = (chy_shp * chr) + (shy_chp * shr); 		y = (shy_chp * chr) - (chy_shp * shr); 		z = (chy_chp * shr) - (shy_shp * chr); 		w = (chy_chp * chr) + (shy_shp * shr); 		return this;
	}
	...
	public Quaternion setFromAxes (boolean normalizeAxes, float xx, float xy, float xz, float yx, float yy, float yz, float zx,
		...
		if (normalizeAxes) {
			...
			zz *= lz;
				...
				if (t >= 0) { 			float s = (float)Math.sqrt(t + 1); 			w = 0.5f * s;
			s = 0.5f / s; 			x = (zy - yz) * s;
			y = (xz - zx) * s;
			z = (yx - xy) * s;
		} else if ((xx > yy) && (xx > zz)) {
			float s = (float)Math.sqrt(1.0 + xx - yy - zz); 			x = s * 0.5f; 			s = 0.5f / s;
			y = (yx + xy) * s;
			z = (xz + zx) * s;
			w = (zy - yz) * s;
		} else if (yy > zz) {
			float s = (float)Math.sqrt(1.0 + yy - xx - zz); 			y = s * 0.5f; 			s = 0.5f / s;
			x = (yx + xy) * s;
			z = (zy + yz) * s;
			w = (xz - zx) * s;
		} else {
			float s = (float)Math.sqrt(1.0 + zz - xx - yy); 			z = s * 0.5f; 			s = 0.5f / s;
			x = (xz + zx) * s;
			y = (zy + yz) * s;
			w = (yx - xy) * s;
		}
	...
	public Quaternion slerp (Quaternion end, float alpha) {
		...
		float scale1 = alpha;
						...
						if ((1 - absDot) > 0.1) {						final float angle = (float)Math.acos(absDot);
