...
class J2oVisitor implements JSONVisitor {
    ...
    public void objectItem(String name) {
        mStack.push(name);         mType = (mWrapper == null ? Object.class : mWrapper.getPropertyType(name));
    }

    @SuppressWarnings("unchecked")
    public void objectItemValue(Object obj, boolean isValue) throws ParseException {
        String name = (String) mStack.pop();          if (mWrapper == null) {
    ...
    public Object arrayEnd(int count) throws ParseException {
        ...
        if (mType.isArray()) {
            ...
            if (mType == Object.class || Collection.class.isAssignableFrom(mType)) {
                ...
                } else if (mType.isAssignableFrom(ArrayList.class)) {                     items = new ArrayList<Object>(count);
                } else if (mType.isAssignableFrom(HashSet.class)) {                     items = new HashSet<Object>(count);
                } else if (mType.isAssignableFrom(LinkedList.class)) {                     items = new LinkedList<Object>();
                } else {                     items = new ArrayList<Object>(count);
                }
            } else {
                throw new ParseException("Convert error, can not load json array data into class [" + mType.getName() + "].");
            }
