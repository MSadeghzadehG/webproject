...
#   include <sys/types.h>
    ...
    #include <sys/stat.h>
    #include <sys/cygwin.h>         #include <cygwin/version.h>
#endif  
#ifdef __BORLANDC__                                             #include <dir.h>
#endif
...
bool wxGetUserName(wxChar *buf, int maxSize)
{
    ...
    if ( !wxGetUserId(szUserName, WXSIZEOF(szUserName)) )
        ...
        WCHAR wszUserName[256];              WCHAR wszDomain[256];
    LPBYTE ComputerName;
...
error:
    ...
    return false;
#else              bool ok = GetProfileString(WX_SECTION, eUSERNAME, wxEmptyString, buf, maxSize - 1) != 0;
    if ( !ok )
    {
...
bool wxDoSetEnv(const wxString& var, const wxChar *value)
{
    ...
    if ( _tputenv(envstr.t_str()) != 0 )
        return false;
#else     if ( !::SetEnvironmentVariable(var.t_str(), value) )
    {
...
#ifdef __VISUALC__
    #pragma warning(push)
    #pragma warning(disable:4996) #endif
...
{
#if defined(__WIN64__)
    return true;  #elif wxUSE_DYNLIB_CLASS         typedef BOOL (WINAPI *IsWow64Process_t)(HANDLE, BOOL *);
