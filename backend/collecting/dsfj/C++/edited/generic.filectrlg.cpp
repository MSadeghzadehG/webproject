...
#include "wx/file.h"        #include "wx/generic/dirctrlg.h" #include "wx/dir.h"
...
#else     const bool hasStat = wxStat( m_filePath, &buff ) == 0;
#endif
...
wxString wxFileData::GetHint() const
{
    ...
    if (IsDir())
        ...
        s += _("<DRIVE>");
    else         s += wxString::Format(wxPLURAL("%ld byte", "%ld bytes", m_size),
...
#if defined(__UNIX__)
            if (p.empty()) p = wxT("/");
#endif             wxFileData *fd = new wxFileData(p, wxT(".."), wxFileData::is_dir, wxFileIconsTable::folder);
            if (Add(fd, item) != -1)
                item.m_itemId++;
            else
                delete fd;
