...
enum
{
    GIF_MARKER_EXT       = '!',     GIF_MARKER_SEP       = ',',     GIF_MARKER_ENDOFDATA = ';', 
    GIF_MARKER_EXT_GRAPHICS_CONTROL = 0xF9,
    GIF_MARKER_EXT_COMMENT          = 0xFE,
    GIF_MARKER_EXT_APP              = 0xFF
};

#define LZ_MAX_CODE         4095    #define FLUSH_OUTPUT        4096    #define FIRST_CODE          4097    
#define HT_SIZE         8192        #define HT_KEY_MASK     0x1FFF      
    ...
    wxOutputStream& stream, bool verbose)
{
    ...
    return wxGIFHandler_GetPalette(*image, pal, &palCount, &maskIndex)
        && DoSaveFile(*image, &stream, verbose, true , 0,
            false , pal, palCount, maskIndex)
    ...
    wxOutputStream *stream, bool verbose, int delayMilliSecs)
{
    ...
    for (i = 0; (i < images.GetCount()) && ok; i++)
    {
        ...
        ok = wxGIFHandler_GetPalette(image, pal, &palCount, &maskIndex)
          && DoSaveFile(image, stream, verbose, i == 0 , delayMilliSecs,
            true , pal, palCount, maskIndex);
    }
...
bool wxGIFHandler::CompressOutput(wxOutputStream *stream, int code)
{
    if (code == FLUSH_OUTPUT)
    {
        ...
        m_crntShiftState = 0;                               if (!wxGIFHandler_BufferedOutput(stream, m_LZBuf, FLUSH_OUTPUT))
        {
            return false;
        }
    }
...
{
    m_LZBuf[0] = 0;               m_clearCode = (1 << bpp);
    m_EOFCode = m_clearCode + 1;
    m_runningCode = m_EOFCode + 1;
    m_runningBits = bpp + 1;         m_maxCode1 = 1 << m_runningBits;           m_crntCode = FIRST_CODE;           m_crntShiftState = 0;          m_crntShiftDWord = 0;
    ...
    const wxUint8 *line, int lineLen)
{
    ...
    wxUint8 pixel;
    if (m_crntCode == FIRST_CODE)                          crntCode = line[i++];
    else
        crntCode = m_crntCode;     
    while (i < lineLen)
    {
                pixel = line[i++];                                            newKey = (((unsigned long) crntCode) << 8) + pixel;
    ...
    bool loop, const wxRGB *pal, int palCount)
{
    ...
    buf[0] |=(bpp - 1) << 5;
    buf[0] |=(bpp - 1);
    buf[1] = 0;     buf[2] = 0;     ok = ok && wxGIFHandler_Write(stream, buf, sizeof(buf))
        && wxGIFHandler_WritePalette(stream, pal, palCount, bpp);
...
{
    return wxGIFHandler_WriteWord(stream, 0)         && wxGIFHandler_WriteWord(stream, 0)         && wxGIFHandler_WriteWord(stream, (wxUint16) width)
        && wxGIFHandler_WriteWord(stream, (wxUint16) height);
}
    ...
    int maskIndex, int delayMilliSecs)
{
    ...
    buf[0] = GIF_MARKER_EXT;        buf[1] = GIF_MARKER_EXT_GRAPHICS_CONTROL;
    buf[2] = 4;         buf[3] = (maskIndex != wxNOT_FOUND) ? 1 : 0;       buf[4] = delay & 0xff;      buf[5] = (delay >> 8) & 0xff;       buf[6] = (maskIndex != wxNOT_FOUND) ? (wxUint8) maskIndex : 0;
    buf[7] = 0;
    return wxGIFHandler_Write(stream, buf, sizeof(buf));
}
