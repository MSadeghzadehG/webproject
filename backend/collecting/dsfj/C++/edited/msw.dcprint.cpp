...
static bool wxGetDefaultDeviceName(wxString& deviceName, wxString& portName)
{
    ...
    pd.lStructSize    = sizeof(PRINTDLG);
    pd.hwndOwner      = (HWND)NULL;
    pd.hDevMode       = NULL;     pd.hDevNames      = NULL;     pd.Flags          = PD_RETURNDEFAULT;
    pd.nCopies        = 1;
...
WXHDC WXDLLEXPORT wxGetPrinterDC(const wxPrintData& printDataConst)
{
    ...
    return 0;
#else     wxWindowsPrintNativeData *data =
    ...
    HDC hDC = ::CreateDC
                (
                    NULL,                                   deviceName.t_str(),
                    NULL,                                   static_cast<DEVMODE *>(lockDevMode.Get())
                );
    if ( !hDC )
    {
